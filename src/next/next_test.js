...
QUnit.test('Defaults', function(){
  ...
  QUnit.deepEqual(vm.attr('params'), {'date':{'$gt': 'test the delay'}, limit: 1, $sort: { date: 1 }}, 'Changing the gtKey updates the params.');
  vm.attr('limitKey', '$limit');

  // additionalParams get mixed in.
  vm.attr('additionalParams', {test: 123});
  QUnit.deepEqual(vm.attr('params'), {'date':{'$gt': 'test the delay'}, $limit: 1, $sort: { date: 1 }, test: 123}, 'Changing the gtKey updates the params.');
